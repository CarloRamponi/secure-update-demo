...
This DEMO shows CROSSCON's Secure firmware update mechanism in action.

The original repositories are:
- https://github.com/crosscon/secure_update_infrastructure/ for the infrastructure
- https://github.com/crosscon/secure_update_consumer/ for everything regarding the update consumer
...
The update affects part of the firmware, particularly bootloader stage 3.2 (BL32), which, in the ARM TF-A's boot sequence, is the secure monitor (i.e. the TEE: OPTEE)
...
Then follow the instructions in its [README](https://github.com/crosscon/secure_update_infrastructure/tree/main/status-server)
...
This host will communicate with the firmware consumer for SBOM verification and remote proof verification. This communication must be authenticated. For this reason, we need a pair of RSA keys.

You can generate them on the device itself, with these commands:

```bash
# Become root and move to its home directory
sudo su
cd /root
# generate the private key and derive the public one
openssl genrsa -out private_key.pem 3072
openssl rsa -in private_key.pem -pubout -out public_key.pem
# copy the pulic key in the tmp folder and make it visible to normal user
cp public_key.pem /tmp
chown user:user /tmp/public_key.pem
```

Then, from the device consumer, you should get the public key and place it in `~/secure_update/keys/`, as stated in its section below.
